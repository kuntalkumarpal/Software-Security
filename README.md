...
* If there is a large buffer to overflow, make sure to repeat NOP followed by address instead of repeating the address over and over again
* Even if a binary is run in the EGG planted shell, the address of the EGG variable may not exactly match, that is why the NOP is usually added
